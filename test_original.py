...
def parse_args():
    ...
    parser.add_argument('--checkpoint', type=str, default="checkpoint_original_model.pth",
...
model = OriginalModel(num_labels=num_labels)

# Load the checkpoint and extract just the model state dict
checkpoint = torch.load(checkpoint_file)
# For the original model, we only need to load the fc3 layer
# The remaining layers will be initialized randomly since we're just testing
model_dict = model.state_dict()
pretrained_dict = {k: v for k, v in checkpoint['model_state_dict'].items() if k in model_dict and k not in ['fc3.weight', 'fc3.bias']}
